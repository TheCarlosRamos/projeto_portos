...
def add_years(d: date, years:int)->date:
    try: return d.replace(year=d.year+int(years))
    except ValueError: return d.replace(month=2,day=28,year=d.year+int(years))
...
def compute_service_fields(row, df00):
    mask=(df00['Zona portuária']==row['Zona portuária']) & (df00['UF']==row['UF']) & (df00['Obj. de Concessão']==row['Obj. de Concessão'])
    cad=df00[mask]
    if cad.empty: return row
    assinatura=_parse_date(cad.iloc[0].get('Data de assinatura do contrato'))
    try: capex_total=float(cad.iloc[0].get('CAPEX Total'))
    except: capex_total=None
    for c in ['Prazo início (anos)','Prazo final (anos)']:
        try: row[c]=int(row.get(c)) if pd.notna(row.get(c)) and row.get(c)!='' else None
        except: row[c]=None
    if assinatura and row['Prazo início (anos)'] is not None:
        row['Data de início']=add_years(assinatura,row['Prazo início (anos)'])
    if assinatura and row['Prazo final (anos)'] is not None:
        row['Data final']=add_years(assinatura,row['Prazo final (anos)'])
    perc=normalize_percentage(row.get('% de CAPEX para o serviço'))
    if perc is not None and capex_total is not None:
        row['CAPEX do Serviço']=round(capex_total*perc,2)
    return row

def validate_cadastro(df00):
    import pandas as pd
    errors=[]
    for idx,r in df00.iterrows():
        t=r.get('Tipo')
        if pd.notna(t) and t not in TIPO_LIST:
            errors.append((idx,'Tipo',f'Valor inválido: {t}'))
        ...
        if pd.notna(d) and _parse_date(d) is None:
            errors.append((idx,'Data de assinatura do contrato','Data inválida'))
        for col in ['CAPEX Total','Coordenada E (UTM)','Coordenada S (UTM)','Fuso']:
            v=r.get(col)
            if pd.isna(v) or v=='': continue
            try: float(v)
            except: errors.append((idx,col,'Valor numérico inválido'))
    return pd.DataFrame(errors, columns=['linha','coluna','erro'])

def validate_servicos(df01, df00):
    import pandas as pd
    errors=[]
    ...
    for idx,r in df01.iterrows():
        ...
        if any(pd.isna(x) or x=='' for x in k):
            continue
        if k not in key00:
            errors.append((idx,'chave (00)','Cadastro (00) não encontrado.'))
        ...
        for col in ['Data de início','Data final']:
            v=r.get(col)
            if pd.isna(v) or v=='': continue
            try: _=pd.to_datetime(v, dayfirst=True)
            except: errors.append((idx,col,'Data inválida'))
    return pd.DataFrame(errors, columns=['linha','coluna','erro'])

def validate_acompanhamento(df02, df01):
    import pandas as pd
    errors=[]
    ...
    for idx,r in df02.iterrows():
        ...
        if any(pd.isna(x) or x=='' for x in k):
            continue
        if k not in key01:
            errors.append((idx,'chave (01)','Serviço (01) não encontrado.'))
        val=r.get('% executada')
        if pd.notna(val):
            if normalize_percentage(val) is None:
                errors.append((idx,'% executada','Porcentagem inválida'))
        v=r.get('Data da atualização')
        if pd.notna(v) and str(v)!='':
            try: _=pd.to_datetime(v, dayfirst=True)
            except: errors.append((idx,'Data da atualização','Data inválida'))
    return pd.DataFrame(errors, columns=['linha','coluna','erro'])
